...
function _java_abs_list_all_param_comment($model)
{
...
function _java_abs_list_all_param($model)
{
    $ii = 0;
    foreach ($model['list_by'] as $key) {
        $ii++;
 ...
 * @return int
 */
function _java_abs_list_all_param4use($model, $in_model = false)
{
    $ii = 0;
 ...
 * @param $model
 */
function java_abs_list_all($model)
{
    ...
    _java_comment_header("查询全部列表，结构为hash map");
    ...
    _java_comment_header("查询全部列表 结构为bean");
    _java_abs_list_all_param_comment($model);
    _java_comment_footer();
