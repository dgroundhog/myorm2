 ...
 * @param $model
 */
function java_mysql_list_basic($model)
{
    ...
    echo _tab(4) . "String sql = \"{CALL `p_{$table_name}_list_basic`({$s_qm})}\";\n";
    ...
    echo _tab(4) . "logger.debug(\"call p_{$table_name}_list--done\");\n";
    ...
    _java_query_header("mysql", "read");
    echo _tab(4) . "String sql = \"{CALL `p_{$table_name}_list`({$s_qm})}\";\n";
    echo _tab(4) . "st = conn.prepareCall(sql);\n";
